...
public:
  ...
  Color shading(Color cm0, Point3D ph, Vector3D nh, View eye, bool no_border){
...
//      /* test end */
  ...
  //cout << t_min << endl;
  if(t_min != FLT_MAX){
    clr = shd.shading(cm0, ph, nh, eye, no_border);
  }
  return clr;
}
...
//  sce.v_view = Vector3D(0, 1, 0);
  ...
  light_src* light_src2 = new light_src(2, Point3D(0, -15, 10), Vector3D(0.2, 1.4, -1), Vector3D( 1, 1, 1), 30);
