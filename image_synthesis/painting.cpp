...
public:
  ...
  Color shading(Color cm0, Point3D ph, Vector3D nh, View eye){
  ...
  Color CalcDiffuse(Color cm0, Point3D ph, Vector3D nh, View eye, Vector3D nhl, int i){
    ...
    if (light[i]->type == 2) {
      t = t * pow((DotProduct(light[i]->direction, nhl) + 1) / 2.0, 4);
...
//  objs.push_back((AnyObject*) cone1);
  ...
  vector<light_src*> light;
  light_src* light_src1 = new light_src(1, Point3D(-30, -10, 20), Vector3D(-1, 1, -1), Vector3D(1, 1, 0.95));
  light.push_back(light_src1);

  light_src* light_src2 = new light_src(2, Point3D(0, -15, 15), Vector3D(0.2, 1.4, -1), Vector3D( 1, 1, 1), 30);
